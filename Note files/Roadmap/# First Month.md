[[Cyber Security/Roadmap/# Cybersecurity Journey - Checklist|# Cybersecurity Journey - Checklist]]

 30-Day Quick Start (Your First Month)

> Days 3-4 already covered via past conversations (process management, OSI model). Use that saved time for extra OverTheWire/Python practice.

### Week 1: Linux & Networking

- [x] Day 1: Install Kali Linux — _Arch already serves this role_
- [x] Day 2: Learn basic Linux commands
- [x] Day 3: Practice file permissions
- [x] Day 4: Study OSI model — _done, with security threat mapping per layer_
- [ ] Day 5: Study TCP/IP model — _done, incl. 3-way handshake & SYN flood_
- [ ] Day 6: Install and use Wireshark
- [ ] Day 7: Practice packet analysis

### Week 2: Programming & Web

- [x] Day 8: Python basics - variables, data types
- [ ] Day 9: Python - functions and loops
- [ ] Day 10: Python - write first script
- [ ] Day 11: How HTTP works
- [x] Day 12: Understand cookies and sessions
- [ ] Day 13: Learn about OWASP Top 10
- [ ] Day 14: Practice with Burp Suite

### Week 3: Security Basics

- [x] Day 15: Study CIA triad — _quick review, overlaps with Cryptography coursework_
- [x] Day 16: Learn encryption basics — _sync with Cryptography coursework_
- [ ] Day 17: Study OWASP Top 10 in detail
- [ ] Day 18: Set up home lab
- [ ] Day 19: Install Metasploitable
- [ ] Day 20: Install Security Onion _(optional — resource-heavy, can defer)_
- [ ] Day 21: Review and practice week's material

### Week 4: Hands-On Practice

- [x] Day 22: Create TryHackMe account
- [x] Day 23: Complete first TryHackMe room
- [x] Day 24: Complete 2 more TryHackMe rooms
- [ ] Day 25: Try first CTF challenge
- [ ] Day 26: Practice with vulnerable VM
- [x] Day 27: Document your learning
- [x] Day 28: Create GitHub account
- [x] Day 29: Set up blog or note-taking system — _Obsidian already installed, use it!_
- [ ] Day 30: Review progress and plan next month